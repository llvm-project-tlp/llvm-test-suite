...
def parse_report(source, filename):
    def check_if_exists(f):
        path = os.path.join(source, 'Fortran', 'gfortran', f)
...
def parse_disabled(source, filename):
    re_unsupported = re.compile('^file\s*[(]GLOB\s+UNSUPPORTED_FILES.*')
    re_unimplemented = re.compile('^file\s*[(]GLOB\s+UNIMPLEMENTED_FILES.*')
    re_erroring = re.compile('file\s*[(]GLOB\s+SKIPPED_FILES.*')
    re_failing = re.compile('file\s*[(]GLOB\s+FAILING_FILES.*')
...
def parse_commandline_args():
    def ancestor(n, curr):
        if n == 0:
            return curr
        return ancestor(n - 1, os.path.dirname(curr))
    ...
    ap.add_argument(
        'source', nargs = '?', default=ancestor(4, __file__),
        help = 'The root of the llvm-test-suite directory. If not provided, '
        'assumes that the script file is in the test suite and determines the '
...
def main():
    ...
    tests = parse_test_configs(args.source)
    unsupported, unimplemented, erroring, failing = \
        parse_disabled_files(args.source)
    ...
    passing, not_passing = parse_report(args.source, args.report)

    # These are tests that are in the configuration, but do not appear in the
    # report. This is usually because they have been excluded as a result of
    # being irrelevant to flang (such as all the tests in regression/graphite,
    # regression/guality and regression/prof), or because they are intended to
    # be run exclusively on a different platform.
    excluded = []
    for t in tests:
        if t in passing or t in not_passing:
            continue
    ...
    # Sanity check. Every file in the disabled lists must either pass, fail
    ...
    print('Summary')
    print('  Passing: {} [{}]'.format(len(passing), len(newly_passing)))
